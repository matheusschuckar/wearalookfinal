...
export default function ManageCouponsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const { data: allowed, error: allowedErr } = await supabase.rpc("partner_email_allowed", { p_email: userEmail });
        ...
        // if storeId provided by query param, try to fetch store_name too (and ensure the partner actually matches)
        if (storeId) {
          // check that partner_email maps to this store (safety)
          const { data: pe, error: peErr } = await supabase
            .from("partner_emails")
            .select("store_name,active")
            .eq("email", userEmail)
            .eq("active", true)
            .maybeSingle<{ store_name: string; active: boolean }>();
          if (peErr) throw peErr;
          if (pe?.store_name) {
            setStoreName(pe.store_name);
            // verify storeId corresponds to that store_name (best-effort)
            const { data: srow, error: sErr } = await supabase
              .from("stores")
              .select("id,store_name")
              .eq("id", storeId)
              .maybeSingle<{ id: number; store_name: string }>();
            if (!sErr && srow) {
              // OK
            } else {
              // fallback: resolve storeId from partner_emails store_name
              const { data: srow2, error: sErr2 } = await supabase
                .from("stores")
                .select("id,store_name")
                .eq("store_name", pe?.store_name || "")
                .maybeSingle<{ id: number; store_name: string }>();
              if (!sErr2 && srow2) {
                setStoreId(srow2.id);
              } else {
                // leave as provided but warn
              }
            }
          }
        } else {
          // resolve store_name from partner_emails and then store.id from stores table
          const { data: pe, error: peErr } = await supabase
            .from("partner_emails")
            .select("store_name")
            .eq("email", userEmail)
  ...
  async function handleDelete(id: string) {
    ...
    try {
      // 1) delete coupon_applicabilities for coupon_id
      const { error: err1 } = await supabase.from("coupon_applicabilities").delete().eq("coupon_id", id);
      if (err1) {
        // log but continue
        console.warn("failed to delete applicabilities:", err1);
      }

      // 2) delete coupon row
      const { error: err2 } = await supabase.from("coupons").delete().eq("id", id).eq("created_by", `brand:${storeId}`);
      if (err2) {
        throw err2;
      }

      // success: refresh page (reload current page)
      setNotice("Cupom excluído com sucesso.");
      // reload current page of data
      // if last item on page was deleted and page > 0, maybe go back a page
